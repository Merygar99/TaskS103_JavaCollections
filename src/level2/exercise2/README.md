...
This task builds upon the previously created Restaurant class. The goal is to implement a custom ordering mechanism so that Restaurant objects are sorted first by their name (alphabetically), and then by rating in descending order when names match.
...
This behavior can be achieved by implementing Comparable or using a custom Comparator, and storing the objects in a TreeSet or a sorted List.
